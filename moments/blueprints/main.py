...
from moments.image_processing import generate_alt_text
...
@main_bp.route('/generate_alt_text/<int:photo_id>', methods=['POST'])
@login_required
def generate_alt_text(photo_id):
    ...
    # Agregar una línea de registro para imprimir el valor de image_path
    current_app.logger.info(f"Generating alt text for image path: {image_path}")

    try:
        alt_text = generate_alt_text(image_path)
        return jsonify({'success': True, 'alt_text': alt_text})
    except Exception as e:
        current_app.logger.error(f"Error generating alt text for photo_id {photo_id}: {e}", exc_info=True)
